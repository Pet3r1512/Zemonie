...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? "fixed top-0 left-0 w-screen h-screen overflow-hidden pointer-events-none" : "w-full h-full"} ${open ? "pointer-events-auto" : ""}`}
    >
      <div
        className={
          (className ? className + " " : "") +
      ...
      >
        ...
        <header
          className={`staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between py-[1.25em]! px-[2em] bg-transparent pointer-events-none z-20 ${headerClassName ?? ""}`}
          ...
          >
            ...
            >
              ...
              >
                {textLines.map((l, i) => (
                  <span
                    className="sm-toggle-line block h-[1em] leading-none text-black dark:text-white!"
                    key={i}
                  >
                    {l}
                  </span>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white dark:bg-gray-950 flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-md pointer-events-auto"
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
...
.dark .sm-scope .sm-socials-link:hover { color: var(--sm-accent, #ff7900); }
.dark .sm-scope .sm-prelayer:nth-child(1) { background: #030712 !important; }
.dark .sm-scope .sm-prelayer:nth-child(2) { background: #111827 !important; }
.dark .sm-scope .sm-prelayer:nth-child(3) { background: #1f2937 !important; }
      `}</style>
    </div>
  );
};
